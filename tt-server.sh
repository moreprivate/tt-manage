#!/usr/bin/env bash
# tt-server.sh — MorePrivate tt-server endpoint manager (run ON the VPS as root).
# Sibling: tt-client-openwrt.sh (MorePrivate tt-client router client; uses clients/<name>.toml from add-user).
...
usage() {
  cat <<EOF
NAME
    tt-server.sh ${VERSION_SCRIPT} — MorePrivate tt-server endpoint (root on this VPS)
...
ensure_creds_file() {
  # Empty registry for tt-server (see parse_clients in tt-server):
...
emit_unit() {
  cat >"${UNIT_PATH}" <<EOF
[Unit]
Description=MorePrivate tt-server endpoint
...
setup_ufw() {
  ...
  ufw allow 443/tcp  # MorePrivate tt-server HTTP/2
  ufw allow 443/udp  # MorePrivate tt-server HTTP/3 / QUIC
...
write_client_config() {
  ...
  cat >"${path}" <<EOF
# MorePrivate tt-server client setup — generated by tt-server.sh add-user
# username=${name}
...
cmd_status() {
  ...
  echo "=== MorePrivate tt-server server status ==="
